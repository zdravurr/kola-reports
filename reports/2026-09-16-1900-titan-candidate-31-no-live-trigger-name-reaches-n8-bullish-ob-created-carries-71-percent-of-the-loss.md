# Titan — **candidate 31: is a TRIGGER NAME worth listening to at all? IT CANNOT BE CONVICTED — but the descriptive answer is sharp.** 🔴 **ZERO of the 9 live trigger names reaches n = 8.** The eleven never-green losers were fired by **FIVE different names**, and **`Bullish OB Created` — 4 entries, 17.4 % of live volume — carries ΣR −3.6745 = 70.9 % of the entire live book's loss**, with the **same sign on paper (n = 9, −4.6067R)**. It is **NOT the score** (ρ ≈ 0) — it **IS the side** (every name maps to exactly one side, 15 of 15). No unsubscription proposed. No diff.

**2026-09-16 19:00 UTC · commit `f16c271` · Titan LIVE REAL MONEY · READ-ONLY PASS · Mercury-SOL NOT TOUCHED** · `openitems_guard` **EXIT=0**
...
**Population: 75 closed positions — 23 LIVE (vpos 86–108), 52 PAPER (vpos 27–85).** vpos 33 excluded (NULL
`initial_risk_usdt`); 6 rows are `archived_pre_geometry_fix` and are not in the closed set. Identical to the
15:20 and 17:30 passes.
...
book). ☠️ **5.5× and 14.4× too large respectively.** Every other p ≥ 0.0432. **Every LIVE p is ≥ 0.0456 and
sits on n = 4.** ☠️ **CONTROL (a) FAILS.**
