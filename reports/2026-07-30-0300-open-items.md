...
| **ADX/ATR/EMA still read the FORMING candle — BY DESIGN, on every TF** | Not a filter and **not a defect to fix** — recorded so it is not rediscovered as a bug | `indicators.py:210-223`, verbatim: *"ATR/ADX/EMA above keep `iloc[-1]` on purpose — they are calibrated on the live forming candle and are out of scope."* The July fix (`55d9c7f`) dropped the forming row for `vol_ratio` **only**; `git diff` against `indicators.py.bak_volforming_20260704` confirms it touched nothing else. Measured effect at vpos 86's entry: **1h −0.59, 15m +0.31, 5m +1.07** (0.3–1.1 pts). Caching (`_CACHE_TTL_BY_TF`, `indicators.py:49`) adds a second staleness layer but is not the driver. |
...
🔴🔴 **Progress: 0 of ~10 — RESTARTED A SECOND TIME at `957f980`, 2026-07-30 02:5x UTC.**
**(Previous restart: `c307bb7`, 2026-07-29 13:21 UTC — see §2.18. That restart is now itself void.)**

**WHY IT RESTARTS AGAIN — and the partition question, answered with data.**
Every exit consult before `625fedc` was handed at least one **inverted** book percentile: the exit
context ranked BingX depth-100 values against the OKX depth-4000 baseline (see §2.19). On vpos 86
the prompt said `Imbalance = 0th pct` for a book at the 62nd-69th and `Opposing wall = 93th pct`
for one at ~50th, **and the advisor quoted the phantom 93rd-percentile wall as a reason to HOLD.**

The operator proposed the right partition: reset consults whose reason **cites a book figure**,
keep those that do not, since the corruption is confined to the book block. **Agreed in principle —
and the data collapses it into a full restart, because the second bucket is EMPTY:**
...
imbalance or book). Sampled verbatim to confirm these are real citations, not loose matching:
...
to make.** The one post-`c307bb7` datapoint (vpos 84, NEUTRAL) is discarded with the rest — its
consults carried the same corrupted block.

**Recorded caveat, so the reasoning is auditable later:** "the reason does not mention the book"
would **not** have proven non-contamination anyway — the prompt is read whole, and a fabricated
...
**Contamination ledger, cumulative:** vpos 82, 83 (pre-`c307bb7`) · vpos 84 (pre-`957f980`) ·
...
Recorded now rather than reconstructed after the fact, so the datapoints exist whichever way this
trade resolves. Position: SHORT 0.0023 BTC @ 63686.0, opened 2026-07-30 00:50:14, 1R = 1081.1 pts,
stop 64767.1 on the exchange. **`EXIT_ADVISOR_DRYRUN = True`, so every one of these was blocked by
design — the advisor cannot close anything.**

| # | time (UTC) | trigger | verdict | conf | position R at verdict | book block |
|---|---|---|---|---:|---:|---|
| 19590 | 00:50:20 | hourly (first, `_st is None`) | `hold` | 0.72 | **−0.01R** | 🔴 corrupted (cited phantom 93rd-pct wall) |
| 19601 | 01:50:24 | hourly | **`close`** | 0.72 | **−0.47R** | 🔴 corrupted |
| 19602 | 02:00:09 | `15m_exit_confirm` | **`close`** | 0.72 | **−0.55R** | 🔴 corrupted |
| 19604 | 02:50:25 | hourly | **`close`** | 0.72 | **−0.51R** | 🔴 corrupted (last one before the fix deployed at 02:51:11) |

**Three consecutive `close` verdicts, all "entry thesis broken/compromised"** — 15m flipped bull,
5m bull/neutral, bullish I-CHOCH and OB created against the short. All four consults carried at
least one inverted percentile, so **none of them count toward §2.4** (they are in the contamination
ledger above). They are logged here as an operational record, not as criterion evidence.

**The first verdict is what §2.4 measures, and for vpos 86 the first verdict was `hold` at −0.01R.**
If this position is eventually stopped out at −1R, that `hold` is the datapoint — not the three
later `close` calls. Recording that now, before the outcome is known, so the bar cannot be moved.
...
🔴 **2026-07-30 — IT REPLICATES AT A LARGER CLEAN n. The study was killed on a sample size that
no longer applies.** On the clean closed cohort (n=53):
...
**LONGs**, so direction is entangled with volume; the sample spans **two sizing eras** ($2000-margin
and $30-margin), so dollar figures are not comparable across it. R-multiples are the fair
comparison. **This is an ENTRY filter and needs its own study — DO NOT BUILD IT FROM THIS TABLE.**
The point of the record is only that the kill was based on n=4 and that basis is gone.
...
### 2.19 ✅ EXIT-SIDE PERCENTILE CROSS-SOURCE — FIXED `625fedc` (2026-07-30), and the guard that keeps it fixed
...
value pins to 0 or 100. Live proof on vpos 86 at 00:50:20: prompt said `Imbalance = 0th pct` (truth
**62nd–69th**) and `Opposing wall = 93th pct` (truth **~50th**); the advisor cited the phantom wall
to justify HOLDing. Depth (`6th pct`) was the one correct figure — it was always read straight from
`orderbook_density`.

**The entry path never had this bug** (`_entry_book_pct`), and that pattern was **copied**, not
...
`entry_ob_imbalance` in `virtual_positions` are **BingX** values written at fill. Pairing them with
an OKX "now" would have re-created the defect one line lower — vpos 86's stored **0.2914** against a
live OKX **0.5008** renders as a dramatic **FLIPPED that never happened**. The entry reference is
therefore taken from the `orderbook_density` row nearest the fill (**21 s** for vpos 86), bounded to
±10 min, with the age printed. **Fixing one seam must not open the next one.**

**STRUCTURAL GUARD — `_exit_pct(col, value, source)`:** `source` is a **required positional
argument** *and* is **ANDed into the SQL WHERE clause**. Required-positional means a caller that has
not thought about provenance cannot write a working call (`TypeError` at the call site). In the
predicate means even a **wrong** source string cannot borrow another book's distribution — it gets
that book's rows or none. `bingx_depth100` matches **zero** baseline rows, so a BingX percentile is
**unobtainable**, not merely omitted. If a second collector source is ever added, every value keeps
ranking against its own distribution with no code change. When a figure cannot be ranked it is
rendered **RAW with its source named and no percentile**, and the prompt says so in words.
...
runs pre-breakeven — so **a zero-point TIGHTEN is the NORMAL outcome, not an edge case.** vpos 86
was the first position to reach that branch since the bound landed, and it hit both unfollowed
consequences:
...
neither the exchange nor the tier ledger; record the advisory and write `t+{tier}_ok` so the later
tiers stay due. Compared **at exchange precision** (`price_to_precision` on both sides), no epsilon:
the first draft used `entry_price * 1e-7`, which is safe at every price this bot has traded but
scales with price while the BTC tick is fixed at 0.1 — **it equals one tick at BTC $1M**, where a
genuine one-tick tighten would read as a no-op.

`'tightened'` **stays terminal for a real move** — that has materially changed the position's
protection. The defect was never that it is terminal; it was that **a no-op was recorded as one**.
**NOT retroactive:** vpos 86 keeps `'tightened'` and no tier fires late — its T+60s/T+300s windows
elapsed ~00:51 and ~00:55, and an `EMERGENCY_CLOSE` on an hour-old health read is not what that tier
is for. Elapsed windows stay skipped **by construction**, not by a special case.
...
extreme book figure of the whole trade**, and the one percentile that was **computed correctly**
even before `625fedc`. It was shown, accurately, to **both** advisors: the entry advisor
(*"Book depth: 2,440 BTC — 6th pct"*) executed at 0.82 confidence; the exit advisor saw it and held.

**No gate reads depth on any path.** It is collected every 60s, rendered into two prompts, stored in
two tables — and weighted by nothing. Either it should carry weight somewhere, or we should stop
rendering it as though it does. **This is a question, not a plan** — it needs its own study, and it
is listed so the asymmetry is not forgotten now that the other book figures have been made honest.
...
- **`LIQUIDITY` → `5M-STRUCTURE`.** The category holds **ten LuxAlgo 5m candlestick signals and
  ZERO order-book inputs**; the book is never scored and never enters `confluence_score`. The line
  appeared on **22 of 73 executed trades** and reads as "the order book was outvoted" — on vpos 86 it
  sent a live-money investigation hunting a book veto that never existed. Renamed via
  `CATEGORY_DISPLAY_LABEL`; the **internal key is untouched** (it is in `matrix_breakdown_json`, read
  by the optimizer, quoted in every past report). **Membership, weights and the minority-zeroing rule
  are deliberately unchanged** — whether that rule should fire at all is still open (§1b of the
  forensics: it silences LIQUIDITY and effectively only LIQUIDITY on trades that execute).
- **`Imbalance: 0.31 (Balanced)`** — hardcoded bands (`<.30/.30-.70/>.70`), no baseline, no source.
  At 00:50:1x the machine held **three different "Imbalance" values in one minute** under one word:
  0.31 (BingX top-20, card), 0.51 (OKX-4000, advisor), 0.2914 (BingX-100, DB). `_imbalance_label`
  **deleted**, not deprecated. No percentile shown, deliberately: this book has no queryable baseline
  of its own, and ranking it against the OKX one is exactly the §2.19 inversion.
- **`Trend Strength (ADX): 25.87`** — no timeframe, and it was the **5m** value, the one TF no gate
  reads. The 1h at that moment was **11.1, below the FLAT floor of 20**; the recheck scored that same
  value −5 eleven seconds later. Now prints `ADX 5m: 25.87 | 1h: 11.12 ← the gates read this`.
